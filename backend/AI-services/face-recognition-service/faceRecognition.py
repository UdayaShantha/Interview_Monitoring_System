...
# Optional: Use FER for emotion detection as an alternative to DeepFace analysis.
# Install via: pip install fer
try:
    from fer import FER
except ImportError:
    FER = None
...
class InterviewMonitoringSystem:
    def __init__(self, candidate_photos: list, emotion_library: str = "deepface"):
        """
        ...
        # Emotion keywords (MediaPipe blendshapes)
        ...
        # DeepFace (or FER) analysis data storage
        self.deepface_data = {
        ...
        if self.emotion_library == "fer":
            if FER is None:
                raise ImportError("FER library not installed. Please install it via 'pip install fer'")
            self.fer_detector = FER()  # Initialize FER detector

    def base64_to_image(self, data) -> np.ndarray:
        """
        Convert base64 string or bytes to an OpenCV image.

        :param data: Base64 encoded string or bytes.
        :return: Decoded image or None if failed.
        """
        try:
            # If data is bytes, assume it's already the raw bytes.
            if isinstance(data, bytes):
                ...
                print("Warning: Unsupported candidate photo format.")
                return None
    ...
    def verify_face(self, frame) -> bool:
        """
        Verify if the face in the frame matches any of the candidate photos.

        :param frame: The image frame from webcam.
        :return: True if verification passes; False if threshold is exceeded.
        """
        try:
            # Skip frames to reduce CPU load unless in alert mode.
            self.frame_count += 1
            if self.frame_count % (self.skip_frames + 1) != 0 and \
    ...
    def calculate_head_rotation(self, face_landmarks) -> float:
        """
        Calculate approximate head rotation from face landmarks.

        :param face_landmarks: List of facial landmark objects.
        :return: Head rotation angle in degrees.
        """
        # Use nose landmark as reference (landmark 1)
        nose = [face_landmarks[1].x, face_landmarks[1].y, face_landmarks[1].z]

        # Use left and right eye positions (landmarks 33 and 263)
    ...
    def update_emotion_scores(self, face_blendshapes) -> None:
        """
        Update emotion scores from MediaPipe blendshapes.

        :param face_blendshapes: List of blendshape groups.
        """
    ...
    def update_tracking(self, detection_result, image_shape) -> None:
        """
        Update tracking data from MediaPipe detection results.

        :param detection_result: Result object from the face detector.
        :param image_shape: Shape of the processed image.
        """
    ...
    def update_deepface_analysis(self, frame) -> bool:
        """
        Perform face verification and facial attribute analysis on the current frame.
        Depending on the selected emotion library, this method either uses DeepFace (default)
        or FER (if specified) for emotion analysis.

        :param frame: The image frame.
        :return: True if processing should continue, False if a verification failure occurred.
        """
        ...
        # First, verify identity (always using DeepFace)
        if not self.verify_face(frame):
            return False

        # Perform emotion (and additional attribute) analysis
        try:
            if self.emotion_library == "fer":
                # Use FER for emotion detection (only emotions are available)
                fer_results = self.fer_detector.detect_emotions(frame)
                if fer_results:
                    # Choose the detection with the highest emotion confidence
                    best_result = max(fer_results, key=lambda res: max(res['emotions'].values()))
                    emotions = best_result['emotions']
                    for emotion, score in emotions.items():
                        if emotion in self.deepface_data['emotions']:
            ...
            print(f"DeepFace/FER analysis error: {str(e)}")
            return True  # Default to true on error

    def _calculate_avg_percentages(self, data_dict: dict) -> dict:
        """
        Calculate average percentages for a dictionary of values.

        :param data_dict: Dictionary containing accumulated values.
        :return: Dictionary with percentage strings.
        """
        total = sum(data_dict.values())
        if total <= 0:
            return {k: "0.00%" for k in data_dict}
        return {k: f"{(v / total * 100):.2f}%" for k, v in data_dict.items()}

    def _calculate_avg(self, data_list: list) -> str:
        """
        Calculate average for a list of numerical values.

        :param data_list: List of numbers.
        :return: Average as a formatted string.
        """
        if not data_list:
            return "0.00"
    ...
    def generate_interview_report(self) -> dict:
        """
        Generate a comprehensive report of all interview metrics.

        :return: Dictionary containing report data.
        """
        # Calculate MediaPipe emotion percentages
        emotion_percentages = {
            k: (v / self.emotion_frame_counts[k] * 100) if self.emotion_frame_counts.get(k, 0) > 0 else 0
        ...
        # Prepare DeepFace (or FER) analysis report
        deepface_report = {
    ...
    def save_report_to_csv(self, report: dict) -> str:
        """
        Save the interview report to a CSV file.

        :param report: Report dictionary.
        :return: File path of the saved report.
        """
        ...
        try:
            with open(filepath, 'w', newline='') as csvfile:
                ...
                for emotion, percentage in report["MediaPipe Emotion Percentages"].items():
                    writer.writerow([emotion, f"{percentage:.2f}%" if isinstance(percentage, float) else percentage])
                writer.writerow([])
...
def run(candidate_photos: list,
    ...
    while cap.isOpened():
        ...
        # Flip image and prepare for MediaPipe processing
...
def main():
    ...
    parser.add_argument('--min_face_presence_confidence',
    ...
    # Note: These candidate photos can be base64 strings or bytes.
    candidate_photos = [
